...
void itoa(uint32_t value)   //integer to string and print it (beacuse print_str can only print string)
{
    char temp[32] = {0};
    temp[31] = '\0';
...
{
    while(1) {
        print_str("willysung@mini-arm-os> ");
        int index = 0;
        while(1) {
            ...
            //"backspace"
            else if(str_buf[index]==127) {   //backspace = 127
                if(index > 0) {
                    str_buf[index--] = '\0';   //erase the last command char (it's in buffer,not showing on shell)
                    print_str("\b \b");        //erase the command showing on shell
                }
            }
...
int main(void)
{
    ...
    const char *username = "willy";

    usart_init();

    //Ready to test for shell
    if (thread_create(my_shell, (void *) username) == -1)
